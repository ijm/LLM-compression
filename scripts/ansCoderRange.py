class AnsCoder:
    ...
    def push(self, symbol, m): # Encodes one symbol.
        ...
        if (self.head >> self.precision) >= m_symbol : # m[symbol]
        ...
        z = self.head % h # m[symbol] + sum(m[0:symbol])
        self.head //= m_symbol # m[symbol]
        self.head = (self.head << self.precision) | z # (This is
            # equivalent to “self.head * n + z”, just slightly faster.)
        print("Pushed : ",symbol, m_symbol, self.bulk)
